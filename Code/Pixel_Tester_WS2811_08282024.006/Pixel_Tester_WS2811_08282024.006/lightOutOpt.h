...
void runLightsInOrder() {
  // Mode 14; LED 1, 2 , 3 etc
  Serial.print("runLightsInOrder: ");
  Serial.print(curColor);
  Serial.print("  |  ");
  Serial.println(inOrderCount);
  ...
  if (curColor == 0) {
    leds[0].r = 250;
    leds[(inOrderCount / 3) + 1].r = 250;
  }
  if (curColor == 1) {
    leds[(inOrderCount / 3) + 1].g = 250;
    leds[0].g = 250;
  }
  if (curColor == 2) {
    leds[(inOrderCount / 3) + 1].b = 250;
    leds[0].b = 250;
  }
  ...
  tft.fillRect(TextBox_X, TextBox_Y, TextBox_W, 50, TFT_WHITE);
  pixelLength = int(EEPROM.read(0));
  tft.drawString(String(inOrderCount), TextBox_X + (TextBox_W / 2), TextBox_Y + (50 / 2));
}

void runLightsInOrderMan() {
  // Mode 14; LED 1, 2 , 3 etc
  Serial.print("manrunLightsInOrder: ");
  Serial.print(curColor);
  Serial.print("  |  ");
  Serial.println(inOrderCount);
  for (int dot = 0; dot < maxPixels; dot++)
