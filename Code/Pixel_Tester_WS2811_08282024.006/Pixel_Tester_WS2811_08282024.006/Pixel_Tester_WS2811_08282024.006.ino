...
void setup() {
  ...
  Serial.println("WS2811_08282024.006");
...
void doClock() {
  ...
  if (timeNow - timeLast >= 100) {
    ...
    if (waitOneSec >= 3 && runCycle == 8 && rgbRunOK == false) {
      rgbRunOK = true;
      waitOneSec = 0;
    }

    if (waitInOrderTim >= 5 && runCycle == 14 && rgbRunOK == false) {
      rgbRunOK = true;
      waitInOrderTim = 0;
    }
...
void loop() {
  ...
  switch (runCycle) {
    ...
    case 13: // BS
      ...
      break;
    case 14:// run in order 1, 2 3, etc
      if (rgbRunOK == true) {
        runLightsInOrder();
      }
      break;
    case 15: //Manual run in order 1, 2 3, etc
      if (rgbManOK == true) {
        runLightsInOrderMan();
      }
      break;
